...
document.querySelector('.gamemplay').addEventListener('click', function(event) {
    ...
    const orderNumberField = document.getElementById('orderNumberField'); //Попытка №
...
document.getElementById('btnOver').addEventListener('click', function (event) { //Описание работы кнопки больше
    event.preventDefault();
    if (gameRun){
...
document.getElementById('btnEqual').addEventListener('click', function (event) { // //Описание работы кнопки верно
    event.preventDefault();
    if (gameRun){
        const phraseRandom = Math.round( Math.random() * 3);
        if (phraseRandom == 0) {
